...
set.seed(15793245) # Reproducibility
...
for (i in 1:n_cross) {
  ...
  for (j in 1:k) {
    ...
    for (l in 1:dim(test)[1]) {
      p[l] <- pred_abacus(abc, test$oSCORAD[l])
    }
...
acl <- data.frame(Acc = easi_range,
                  Acc_lower = (easi_range - MCID) * (easi_range > MCID),
...
for (i in 0:83) {
  os_to_easi[i + 1, "EASI"] <- pred_abacus(abc, i)
}

ggplot(data = os_to_easi, aes(x = oSCORAD, y = EASI)) +
  geom_point() +
  theme_bw(base_size = 15)
...
for (i in 0:72) {
  easi_to_os[i + 1, "oSCORAD"] <- pred_abacus(abc, i)
}

ggplot(data = easi_to_os, aes(x = EASI, y = oSCORAD)) +
  geom_point() +
  theme_bw(base_size = 15)
