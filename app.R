...
plot_conv <- function(df,xlab){
  if (xlab=="EASI"){
    p=ggplot(data=df,aes(x=EASI,y=oSCORAD))+lims(x=c(0,72),y=c(0,83))
  }else{
    p=ggplot(data=df,aes(x=oSCORAD,y=EASI))+lims(x=c(0,83),y=c(0,72))
  }
  p=p+geom_point(size=2)+theme_bw(base_size = 15)
  if (ncol(df)>2){
    ...
    tabPanel("Stratification",
             sidebarLayout(
               sidebarPanel(
                 ...
                 # Options heatmap
                 radioButtons(inputId = "disp_iga",label = "Values on display",
...
server <- function(input, output,session) {
  ...
  df=reactive({
    req(input$up_df)
    return(read.csv(input$up_df$datapath,header = input$header,sep = input$sep,na.strings = c(strsplit(input$na,",")[[1]],"","NaN")))
  })
  
  # Display input data
  output$disp_data=renderTable({df()})
  ...
  df2=reactive({
    ...
    if (input$score_select=="None"){res=data.frame()}else{
  ...
  output$plot1=renderPlot({
    req(isvalid())
    if (isvalid()){
      if (input$conversion=="from oSCORAD to EASI"){plot_conv(df2(),"oSCORAD")}else{plot_conv(df2(),"EASI")}
    }
  ...
  output$id_name <- renderUI({
    selectInput("ID_select", "Select patient ID (optional):", c("None",colnames(df())))
  })
  
  # Input selection
  output$score <- renderUI({
    selectInput("score_select", "Select score:", c("None",colnames(df())))
  })
  ...
  observeEvent(input$help_button1, {
    if ((input$help_button1+1) %% 2){btn_lbl="Hide this page"}else{btn_lbl="Show more"}
...
If not, please double-check what you did during the previous steps, especially what the strings that should treated as missing.
  ...
  output$sl1 <- renderUI({
    if (input$cutoffs!="Default"){
      if (input$cutoffs=="Same"){lbl="Choose a cut-off"}else{lbl="\"Clear\" vs \"Almost clear\""}
      sliderInput(inputId = "sl1",label = lbl,value = 0.5,min = 0,max = 1)
    }
  })
  
  output$sl2 <- renderUI({
    if (input$cutoffs=="Personalised"){
      sliderInput(inputId = "sl2",label = "\"Almost clear\" vs \"Mild\"",value = 0.5,min = 0,max = 1)
    }
  })
  
  output$sl3 <- renderUI({
    if (input$cutoffs=="Personalised"){
      sliderInput(inputId = "sl3",label = "\"Mild\" vs \"Moderate\"",value = 0.5,min = 0,max = 1)
    }
  })
  
  output$sl4 <- renderUI({
    if (input$cutoffs=="Personalised"){
      sliderInput(inputId = "sl4",label = "\"Moderate\" vs \"Severe\"",value = 0.5,min = 0,max = 1)
    }
  })
  
  output$sl5 <- renderUI({
    if (input$cutoffs=="Personalised"){
      sliderInput(inputId = "sl5",label = "\"Severe\" vs \"Very severe\"",value = 0.5,min = 0,max = 1)
    }
  })
  
  ct=reactive({
    res=0:4
    if (input$cutoffs=="Default"){res=res+0.5}else{
      if (input$cutoffs=="Same"){res=res+input$sl1}else{
  ...
  xi=reactive({
    if (input$disp_iga=="no"){res=c()}else{
      if (input$disp_iga=="only"){res=c(0,5)}else{
        res=0:5
      }
    }
    return(res)
  ...
  observeEvent(input$help_button2, {
    if ((input$help_button2+1) %% 2){btn_lbl="Hide this page"}else{btn_lbl="Show User Guide"}
